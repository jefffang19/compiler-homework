...
simple			: name assign ';' {
				...
				| READ '(' name ')' ';' { 
					...
					else sprintf($$,"%s(%s) ;>'%s' id did not exist",$1,$3,$3);
				}
				| name PPLUS ';' {
					$$ = (char*)malloc(sizeof(char)*returnDollarLEN);
					if(check_exist($1)) sprintf($$,"%s%s ;",$1,$2);
					else sprintf($$,"%s%s ;>'%s' id did not exist",$1,$2,$1);
				}
				| name MMINUS ';' { 
					$$ = (char*)malloc(sizeof(char)*returnDollarLEN);
					if(check_exist($1)) sprintf($$,"%s%s ;",$1,$2);
